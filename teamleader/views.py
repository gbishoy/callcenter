...
def get(request):
    branch = Cfmerchant.objects.raw("SELECT distinct(branch_name) as id ,count(branch_name) as cusnum  FROM dbo.dis_in_client_damen_cf group by branch_name order by branch_name ")
    ...
    emp = Cfmerchant.objects.raw("select officer_name  ,count(officer_name) as id from dbo.dis_in_client_damen_cf where add_datetime < CAST( GETDATE() AS Date ) group by officer_name ")

    if request.POST.get('customerdata') :
        branchname = request.POST['customerdata'] 
        branch = Cfmerchant.objects.raw("SELECT distinct(branch_name) as id ,count(branch_name) as cusnum  FROM dbo.dis_in_client_damen_cf group by branch_name order by branch_name ")
        emp = Cfmerchant.objects.raw("select officer_name  ,count(officer_name) as id from dbo.dis_in_client_damen_cf where  branch_name = '{}' and add_datetime < CAST( GETDATE() AS Date ) group by officer_name ".format(branchname))
